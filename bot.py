...
async def ask_ai(prompt, user_id):
    ...
    sys_instruction = (
        ...
        "3. Стиль: Профессиональный, лаконичный.\n"
        "Обязательно используй ☁️ и 🔴 для структуры."
    )
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not user['name']:
        ...
        return await update.message.reply_text("☁️ Senior Mentor на связи. Как твое имя?")
...
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    await db_mod("UPDATE users SET last_seen = datetime('now') WHERE id = ?", (uid,))
    user = await db_get("SELECT * FROM users WHERE id = ?", (uid,))
    
    if user and user['state'] == 'wait_name':
        await db_mod("UPDATE users SET name = ?, state = NULL WHERE id = ?", (update.message.text[:20].strip(), uid))
        return await update.message.reply_text("🔴 Инициализация завершена. Жми /start.")
