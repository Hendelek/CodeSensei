...
> An intelligent, asynchronous mentorship system for Telegram that leverages LLMs for adaptive, personalized learning.
...
| Feature | Description |
| :--- | :--- |
| **Adaptive Diagnostic Testing** | Conducts a technical interview (4 structured questions). Upon completion, securely parses AI outputs into strict JSON to automatically assign the correct starting topic index. |
| **Smart Context Management** | Maintains an optimized sliding context window of the last 6 dialogue turns stored as JSON in SQLite to preserve continuity without exceeding token limits. |
